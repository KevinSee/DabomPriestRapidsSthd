...
# which dam counts are we using for DABOM?
dam_cnt_name <-
  c("PriestRapids",
    "RockIsland")[2]
```
...
gen_files <- all_file_nms[str_detect(all_file_nms, "^Gen")]
gen_file_nm <-
  tibble(file_nm = gen_files) |> 
  mutate(all_yrs = str_extract_all(file_nm, "[:digit:]+"),
         min_yr = pluck(all_yrs, 1)[1],
         max_yr = pluck(all_yrs, 1)[2],
         across(ends_with("yr"),
                as.numeric)) |> 
  slice_max(max_yr) |> 
  pull(file_nm)
...
#                        "Genetics_PBT_GSI",
#                        "RY2025_SY2026",
#                        "2026_07_30 Douglas - AppendixI_OmyPRD2025(Jul01-Oct31).xlsx"),
#              sheet = "PBT_GSI",
#              skip = 1) |>
#   clean_names() |>
...
tag_summ <-
  ...
  mutate(across(origin,
                ~ case_match(.,
                             ...
                             "H" ~ "HOR",
                             .default = .))) |> 
  filter(assignment_method %in% c("GSI", "PBT")) |> 
...
```{r tag-rate}
tag_rate <-
  query_dabom_results(dabom_dam_nm = dam_cnt_name,
                      query_year = yr,
...
# furthest upstream dam reached by overshoot fish
ovrst_tags <-
  all_dets |> 
  filter(ovrst) |> 
  group_by(tag_code,
...
```{r}
#| label: tbl-flbk
#| tbl-cap: The number of tags overshooting into the Upper Columbia last detected at each dam, overshoot abundance (standard error) of adult steelhead, by origin, the abundance estimated to fallback to a tributary somewhere downstream of Priest Rapids Dam (standard error), and the percent of fallback success. Fish are broken out by the furthest Upper Columbia dam they are detected overshooting past, but all fallback must be downstream of Priest Rapids Dam. 
...
```{r}
#| label: tbl-flbk-yak
#| tbl-cap: The number of adult tags, originating from the Yakima watershed, that overshot into the Upper Columbia and were last detected at each dam, overshoot abundance (standard error) of adult steelhead from the Yakima, the abundance estimated to fallback into the Yakima, and the percent of fallback success. Fish are broken out by the furthest Upper Columbia dam they are detected overshooting past, but all fallback must be within the Yakima basin. 
